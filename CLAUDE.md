...
- **SharedComponents** — Platform-agnostic SwiftUI views. No UIKit dependency. Organized into `Models/`, `Services/`, `Views/` (e.g. `ErrorView`, `HeroView`, `LogoVersionView`, `PowerUserView`, `RatingsView`, `StatusBannerView`, `TipOfTheDayView`), `Models/Transit/` (agency-agnostic transit domain models — `TransitNetwork`, `TransitLine`, `TransitStop`, `Departure`, `Coordinate` — referencing each other by ID rather than embedding full values, resolved through the `TransitDataProvider` protocol in `Services/`), `Views/Transit/` (small, stateless transit-domain atoms: `Amenities`, `BikeIcon`/`ParkingIcon`/`RestroomIcon`, `ZoneTextView`, `TrainLogo`), `Views/ServiceAlerts/` (`ServiceAlertsView`, `ServiceAlertRow`, `AllServiceAlertsView`, `ServiceAlertDetailView` — a closure-driven alert display; navigation/presentation is left to the host app, not baked in), and `Resources/` (color assets in `Colors.xcassets`). `CloudSettings.swift` lives at the target root.
